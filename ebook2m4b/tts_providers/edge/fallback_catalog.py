...
FALLBACK_EDGE_VOICES: tuple[str, ...] = (
    "af-ZA-AdriNeural",
    "af-ZA-WillemNeural",
    "am-ET-AmehaNeural",
    "am-ET-MekdesNeural",
...
def fallback_voice_rows() -> tuple[dict, ...]:
    rows: list[dict] = []
    for voice_id in FALLBACK_EDGE_VOICES:
        parts = voice_id.split("-")
        locale = "-".join(parts[:2]) if len(parts) >= 2 else ""
        locale_name = locale or "Unknown"
        rows.append(
            {
                "ShortName": voice_id,
                "FriendlyName": voice_id,
                "Locale": locale,
